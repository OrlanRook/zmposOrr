...
import { IReentry, IReentryItem } from './re-entry.interface';
import { ReentryService } from './re-entry.service';
...
export class ReEntryComponent implements OnInit {
    ...
    this.brachService.getAll().subscribe({
      next: (items: IBranch[])=>{
        ...
        this.itemService.getAll().subscribe((data: IItem[])=>{
          this.dataMasterBinding = data;

          this.dataBinding = this.dataMasterBinding.filter((obj)=> {
            let branch = obj.product_stock.find(obj2 => obj2['branch'] === this.currentBranch.id );
            if( branch ) {
              return true;
            }
            else {
  ...
  updateTable() {
    this.reentryService.getAll().subscribe({
      next: (items: IReentry[])=>{
  ...
  getStock(item: IItem) {
    const inventory = item.product_stock.find((obj:IInventory) => { return obj.branch === this.currentBranch.id });
    return inventory?.stock;
  }

  setStock(item: IItem, quantity: number) {
    var inventory = item.product_stock.find((obj:IInventory) => { return obj.branch === this.currentBranch.id });
    if( inventory ) {
  ...
  makeTransfer() {
    ...
    let items: IReentryItem[] = []
    this.dataCart.forEach(function (x) {
      let data: IReentryItem = {
        product:    x.id,
        stock:      x.quantity
      };
      items.push(data);
    ...
    this.reentryService.createTransfer(transfer).subscribe({
      next: (items: IReentry)=>{
